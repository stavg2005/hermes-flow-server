...
http::request<http::empty_body> create_signed_get_request(
  ...
  aws_sigv4::Signature signer(cfg.service, full_host, cfg.region,
                              cfg.secret_key, cfg.access_key, now);

  // Empty SHA-256 hash (for GET requests)
  std::string payload_hash =
      "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";

  // We manually format the date here to ensure consistency with the signer's
  // internal time
  char amzdate[20];
  std::strftime(amzdate, sizeof(amzdate), "%Y%m%dT%H%M%SZ", &timeinfo);

  // to comply with the library format we use std::map<std::string,
