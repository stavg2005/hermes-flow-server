...
std::optional<boost::span<uint8_t>> RTPPacket::to_buffer(
  ...
  const size_t extension_size =
      header.extension ? MINIMUM_EXTENSION_SIZE +
                             header.extension->data.size() * sizeof(uint32_t)
                       : 0;

  size_t buffer_size =
      MINIMUM_HEADER_SIZE + csrc_list_size + extension_size + payload.size();
  ...
  const FirstOctet first_octet = {static_cast<uint8_t>(header.csrc_list.size()),
                                  header.extension.has_value(), header.padding,
                                  header.version};
...
std::optional<RTPPacket> RTPPacket::from_buffer(
    const boost::span<uint8_t>& buffer) {
